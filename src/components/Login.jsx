...
function Login() {
  ...
  const postForm = async (data) => {
    try {
      ...
      if (res.success) {
        const me = await fetch(`${BACKEND_URL}/api/user/me`, {
          credentials: "include"
        });
        const meData = await me.json();
  ...
  return (
    <form onSubmit={handleSubmit(postForm)} className='align-middle'>
      ...
      {/* SERVER MESSAGE */}
      {servermsg && (
        <div className='bg-blue-200 mt-5 w-full p-3 rounded-4xl text-center'>
          <span>{servermsg}</span>
        </div>
      )}
      ...
      <div className="w-full flex justify-center items-center">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="relative w-full sm:w-2/3 md:w-1/2 lg:w-1/3 min-h-[50vh] bg-white flex flex-col justify-evenly items-center rounded-2xl m-5 shadow-[6px_6px_10px_#a3a3a3,-8px_-8px_14px_#ffffff]"
        >
          ...
          <div
            className="bg-white rounded-xl px-8 py-2 text-lg text-green-400 font-bold shadow-[inset_3px_3px_5px_#a3a3a3,inset_3px_3px_4px_#ffffff]"
          >
            <input
          ...
          <div>
            <span>Create a new account</span>
            <Link to="/signup" className='text-blue-400 underline ml-3'>
              Signup
            </Link>
          </div>
